...
struct FormTextArea: View {
    ...
    init(title: String, name: Binding<String>, error: String, max: Int = 120, showCount: Bool = false) {
        self.title = title
        self._name = name
        self.error = error
        self.max = max
...
struct FormTextArea_Previews: PreviewProvider {
    static var previews: some View {
        FormTextArea(title: "first name", name: .constant("name"), error: "", showCount: true)
    }
}
