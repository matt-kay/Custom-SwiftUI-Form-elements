...
struct FormPhoneNumberField: View {
    let title : String
            ...
            VStack{
                List(NSLocale.isoCountryCodes, id : \.self){ countryCode in
                    if countryDictionary[countryCode] != nil {
                        Button {
                            if let code = countryDictionary[countryCode]{
                                selectedCode = code
                            }
                            show.toggle()
                        } label: {
                            HStack{
                                ...
                                Spacer()
                                Text("+\(countryDictionary[countryCode] ?? "not")")
                            }
                        }
...
struct FormPhoneNumberField_Previews: PreviewProvider {
    static var previews: some View {
        FormPhoneNumberField(title: "Enter phone number", name: .constant("45678909876"), selectedCode: .constant("234"), error: "")
    }
}
